...
export default class Table {
  ...
  addCovidDataToTable(cases, deaths, recovered) {
    const covidDataItemCases = this.getTableItemElements(
      'Coronavirus Cases',
      cases
    );
    const covidDataItemDeaths = this.getTableItemElements('Deaths', deaths);
    const covidDataItemRecover = this.getTableItemElements(
      'Recovered',
      recovered
    );
    this.covidDataContainer.innerHTML = '';
    this.covidDataContainer.append(
      ...covidDataItemCases,
      ...covidDataItemDeaths,
      ...covidDataItemRecover
    );
  }

  getTableItemElements(name, value) {
    const covidDataItemName = generateElement(
      'div',
      'covidData-container__item_name',
      name || UNKNOWN
    );
    const covidDataItemValue = generateElement(
      'div',
      'covidData-container__item_value',
      value || UNKNOWN
    );
    const covidDataItemDots = generateElement(
      'div',
      'covidData-container__item_dots' || UNKNOWN
    );
